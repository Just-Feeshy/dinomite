...
class FeshSprite extends FlxSprite {
    ...
    */
    ...
    /*
    * Shit's in radians lol
    * @return an angle when reached over 180 displays as negative and modded by 360 in radians.
    */
    public function getProperAngle():Float {
        var returnAngle:Float = 0;

        if(angle > Math.PI)
            returnAngle = (angle % (Math.PI * 2)) - (Math.PI * 2);
        else
            returnAngle = (angle % (Math.PI * 2));
